...
public class FileUtils implements DefaultLifecycleObserver {
  ...
  private void saveDirectory(Uri dirUri) {
    ...
    resolver.takePersistableUriPermission(
        dirUri, Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
  }
  ...
  public ArrayList<Rom> listRoms() {
    ...
    if (treeUri != null) {
      int compressionLevel =
          sharedPref.getInt(context.getString(R.string.saved_compression_level_key), 1);
      boolean deleteAfter =
          sharedPref.getBoolean(context.getString(R.string.saved_delete_after_key), false);
      ...
      if (romsFolder != null) {
        ...
        for (DocumentFile file : romsFolder.listFiles()) {
          if (file.isFile()) {
            ...
            if (inputMimeType.equals(fileMimeType)
                && inputFileExtension.equals(fileExtension.toLowerCase())) {
              final Uri inputUri = file.getUri();
              final String inputFilename = file.getName();
              final String outputFilename =
                  inputFilename.replaceFirst(inputFileExtension + "$", outputFileExtension);
      ...
      Toast.makeText(context, context.getString(R.string.no_roms_folder), Toast.LENGTH_SHORT)
          .show();
    }
