...
public class RecyclerViewUtils {
  ...
  private void onItemClick(int position) {
    ...
    if (clickedRom.getStatus().equals(context.getString(R.string.status_added))) {
      EditRomDialog dialog =
          new EditRomDialog(clickedRom, isDecompression, () -> adapter.notifyItemChanged(position));
  ...
  public void reloadRomsSettings() {
    int compressionLevel =
        sharedPreferences.getInt(context.getString(R.string.saved_compression_level_key), 1);
    boolean deleteAfter =
        sharedPreferences.getBoolean(context.getString(R.string.saved_delete_after_key), false);
    for (Rom rom : roms) {
